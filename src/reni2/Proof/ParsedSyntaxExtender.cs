...
namespace Reni.Proof
{
    ...
    {
        private static readonly IComparer<IOrderedEnumerable<string>> _comparer = new ComparerClass();

        private sealed class ComparerClass : IComparer<IOrderedEnumerable<string>>
        {
            public int Compare(IOrderedEnumerable<string> x, IOrderedEnumerable<string> y)
            {
                ...
                {
                    var result = xx[i].CompareTo(yy[i]);
                    if (result != 0)
                        return result;
                }
        ...
        private static ParsedSyntax ComineCheck(IAssociative @operator, ICollection<ParsedSyntax> result, ParsedSyntax current, ParsedSyntax parsedSyntax)
        {
