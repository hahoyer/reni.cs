...
sealed class ListStack : StackData
{
    ...
    internal override Size Size => Data.Aggregate(Size.Zero, (current, data) => current + data.Size);
    ...
    DataStack.DataMemento GetItemMemento(NonListStackData item, int index) => new(item.Dump())
    {
        Size = item.Size.ToInt(), Offset = Data.Skip(index).Sum(item1 => item1.Size.ToInt())
    };
